...
BOOL btl_scr_cmd_101_strengthsapcalc(void *bw, struct BattleStruct *sp);
BOOL btl_scr_cmd_102_didtargetraisestat(void *bw, struct BattleStruct *sp);
...
const btl_scr_cmd_func NewBattleScriptCmdTable[] =
{
    ...
    [0x101 - START_OF_NEW_BTL_SCR_CMDS] = btl_scr_cmd_101_strengthsapcalc,
    [0x102 - START_OF_NEW_BTL_SCR_CMDS] = btl_scr_cmd_102_didtargetraisestat,
};
...
/**
 *  @brief script command to set toxic spikes as a result of Toxic Debris. normal TryToxicSpikes specifically checks for the attacker so we can't use that
 *
 *  @param bw battle work structure
 *  @param sp global battle structure
 *  @return FALSE
 */
BOOL btl_scr_cmd_FE_trytoxicdebris(void *bw UNUSED, struct BattleStruct *sp) {
    IncrementBattleScriptPtr(sp, 1);

    int address = read_battle_script_param(sp);
    int side = IsClientEnemy(bw, sp->defence_client)^1;

    if (sp->scw[side].toxicSpikesLayers == 2) {
        IncrementBattleScriptPtr(sp, address);
    } else {
 ...
 *  @return FALSE
 */
BOOL btl_scr_cmd_101_strengthsapcalc(void *bw UNUSED, struct BattleStruct *sp) {
    IncrementBattleScriptPtr(sp, 1);
 ...
 *  @return FALSE
 */
BOOL btl_scr_cmd_102_didtargetraisestat(void *bw UNUSED, struct BattleStruct *sp) {
    IncrementBattleScriptPtr(sp, 1);
