            ...
            case SWITCH_IN_CHECK_SLOW_START:
                for (i = 0; i < client_set_max; i++)
                {
                    ...
                    if ((sp->battlemon[client_no].slow_start_end_flag == 0)
                        && (sp->battlemon[client_no].hp)
                        && (GetBattlerAbility(sp, client_no) == ABILITY_SLOW_START)
                        && ((sp->total_turn-sp->battlemon[client_no].moveeffect.slowStartTurns) == 3))
                    {
        ...
        */
        ...
        case ABILITY_BERSERK:
            ...
            (
                ...
                && !((GetBattlerAbility(sp, sp->attack_client) == ABILITY_SHEER_FORCE) && (sp->battlemon[sp->attack_client].sheer_force_flag == 1)) // Berserk doesn't activate if attacked by sheer force
            )
            {
                if
                (
                    ...
                    &&
                    (
                        /* physical_damage and special_damage contain the relevant damage value, but the value is negative */
